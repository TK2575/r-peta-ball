...
```{r Step 2}
iso_coefficient <- -11.72095212
slg_coefficient <- 6.6236732
obp_coefficient <- -8.192095152
intercept_coefficient <- 3.715037045
...
team_stats_2020 <-
  ...
  mutate(hits_per_run = 
         ...
         hits_a_per_run = 
           ...
           intercept_coefficient,
         runs_exp = (h / hits_per_run + baseruns)/2 %>% round(),
         runs_a_exp = (ha / hits_a_per_run + baseruns_a)/2 %>% round()) # losing accuracy by rounding on intermediate steps

step2 <-
  team_stats_2020 %>% 
  mutate(r = (runs_exp - r_2020) * (games_2021 / g) %>% round(), # positive value means they were unlucky (runs were expected higher)
         ra = (ra_2020 - runs_a_exp) * (games_2021 / g) %>% round()) %>% # positive value means they were unlucky (runs allowed were expected lower)
   #FIXME the r_g initial stat in 2020 and all the rounding now gets inflated with the 162/60 correction. 
  # is it possible to view intermediate tables as rounded, but keep the variables un-rounded?
  select(tm, r, ra) 

...
games_this_year <- 162
ex <- 1.83
last_ten_years <- c(2011:2019,last_year)
...
team_batting_last_year <- 
  team_batting(last_year) %>% 
...
batting_results_last_year <-
  here::here("in", "last_year_batting_results.csv") %>% 
  readr::read_csv() %>% 
  rename(baseruns = BsR) %>% 
  janitor::clean_names() %>% 
  left_join(team_abbreviation_map) %>% 
  select(tm, war, baseruns) %>% 
  left_join(team_batting_last_year) %>% 
  select(-year)
...
team_batting_against_last_year <- 
  team_batting_against(last_year) %>% 
  filter(!tm %in% c('LgAvg','LgTot')) %>% 
  janitor::clean_names() %>% 
  select(year, tm, r, h, ba, obp, slg) %>% 
  mutate(r = as.numeric(r),
...
pitching_results_last_year <-
  here::here("in", "last_year_pitching_results.csv") %>% 
  readr::read_csv() %>% 
  rename(baseruns_a = BsR) %>% 
  janitor::clean_names() %>% 
  left_join(team_abbreviation_map) %>% 
  select(tm, war, baseruns_a) %>% 
  left_join(team_batting_against_last_year) %>% 
  select(-year)
...
team_stats_last_year <-
  standings_last_year %>% 
  full_join(batting_results_last_year, by = c("tm")) %>% 
  select(-war) %>% 
  full_join(pitching_results_last_year, by = c("tm")) %>% 
  select(-war)
...
odds_table <-
  here::here("out",
             "regular_season_win_total_odds_this_year.csv") %>% 
  read_csv() %>% 

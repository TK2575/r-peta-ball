...
pitching_results_2020 <-
  ...
  janitor::clean_names() %>% 
  left_join(team_abbreviation_map) %>% 
  select(tm, war) %>% left_join(team_batting_against_2020) %>% 
  select(-year)
...
team_stats_2020 <-
  ...
  mutate(hits_per_run = 
         ...
         hits_a_per_run = 
           ...
           intercept_coefficient,
         runs_exp = (h / hits_per_run) %>% round(),
         runs_a_exp = (ha / hits_per_run) %>% round()) 

step2 <-
  team_stats_2020 %>% 
  # FIXME runs_exp appear correct, runs_a_exp do not
  mutate(r = (runs_exp - r_2020) * (games_2021 / g) %>% round(),
         ra = (ra_2020 - runs_a_exp) * (games_2021 / g) %>% round()) %>% 
  select(tm, r, ra) 

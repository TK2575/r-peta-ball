...
step2 <-
  team_stats_2020 %>% 
  mutate(r = ((runs_exp - r_2020) * (games_2021 / g)) %>% round(2), # positive value means they were unlucky (runs were expected higher)
         ra = ((ra_2020 - runs_a_exp) * (games_2021 / g)) %>% round(2)) %>%  # positive value means they were unlucky (runs allowed were expected lower)
  select(tm, r, ra) 
...
# FIXME if this chunk is run multiple times, step 3 yields different results.
# FIXME need a way to check war diff against RS and RA by viewing team_projections_2021. but team_projections_2021 has been reduced to 2 columns. need to see a snapshot of the whole table to check, and the snapshot shouldn't change each time the chunk is run.

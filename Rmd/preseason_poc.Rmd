...
runs_pace_2020 <- #how many runs would be scored by all teams in 162 games
  team_stats_2020 %>% 
...
runs_projected_adjustments <-
  tibble(r = (runs_pace_2020$r - runs_projected_2021$r) / team_count, #total runs (per team) scored last year minus this year
...
team_projections_2021 <-
  ...
  rename(r_raw = r,
         ra_raw = ra) %>% 
  mutate(r = r_raw + runs_projected_adjustments$r, #add (or subtracts) runs to make the "run environments" the same between last year and this year
